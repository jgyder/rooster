...
Object.defineProperty(window, 'brwsr_def', {
...
Object.defineProperty(window, 'brwsr_req', {
...
brwsr_def('json3_cleanup', [], function() {
  if(window.JSON &&
    (!JSON.parse || JSON.parse.toString().indexOf('[native code]'))) {
    window.JSON = null;
  }
});

brwsr_def('array_cleanup', [], function() {
  var
  ArrayProto = Array.prototype,
  arrayOverrides = ['forEach', 'map', 'reduce', 'reduceRight', 'filter', 'every',
    'some', 'indexOf', 'lastIndexOf'],
  i, name, fn;
  for(i = 0; i < arrayOverrides.length; i += 1) {
    name = arrayOverrides[i];
    fn = ArrayProto[name];
    if(fn && (fn.toString().indexOf('[native code]') < 0)) {
      // Non-native function, delete for compatibility
      delete ArrayProto[name];
    }
  }
});

brwsr_def('zepto_pre', [], function() {
  window.oldZepto = window.Zepto;
});

brwsr_req(['json3'], function() {});
...
EL_BRIDGE = document.createElement('p');
...
var old = document.getElementById(ID_BRIDGE);
if(old) {
...
addEventListener('DistillEventExt', function() {
  var msg = JSON.parse(EL_BRIDGE.textContent);
...
function onMessage(msg) {
  if(msg.type == MSG_REQUEST) {
    //DBG && console.log('CONTENT: request:', msg._id, msg.path, msg);
...
function __brwsr__request(_id, msg) {
  ...
  var options = _.isString(msg) ? JSON.parse(msg) : msg;

  setTimeout(function() { callApi(options, callback) }, 0);
...
function callApi(options, callback) {
  var api = Api[options.path] || Api.none, 
    input = options.data;
...
var EVENTS = ['abort', 'beforeunload', 'blur', 'change', 'click', 'close',
...
brwsr_def('id', [], function() { return ID });

brwsr_def('api', ['underscore', 'json3'], function(_) {
  return {
...
function onWindowMessage(event) {
  var
  data = event.data,
  type = data.brwsr_type, // We identify out messages using name brwsr_type.
  id = data.id,
  path = data.path,
  source = event.source;
...
brwsr_def('util', ['underscore', 'api'], function(_, Api) {
  ...
  Api.addRequestHandler('frame/offset', function(event, callback) {
    var el = findWindowsFrame(event.source);
    if(!el) {
  ...
  function findWindowsFrame(aWindow) {
    var frames = document.querySelectorAll('iframe,frame'), frame;
  ...
  function getOffsetFromScreen(el, callback) {
    var offset = _.clone(el.getBoundingClientRect());
